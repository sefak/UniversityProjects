fid=fopen('Circuit1.txt', 'r');
r=0; v=0; i=0; %# of R, V, and I, respectively.
resistors={zeros(100,1), zeros(100,2)};%1st field of cell array is the value of R, and 2nd field is the nodes of R.
vsources={zeros(100,1), zeros(100,2)};%1st field of cell array is the value of V, and 2nd field is the nodes of V.
isources={zeros(100,1), zeros(100,2)};%1st field of cell array is the value of I, and 2nd field is the nodes of I.
while feof(fid)==0
...
n=max([max_r max_v max_i]);% of nodes
...
for a = 1:n
    for b = 1:n
        if a==b%at diagonal of G, the value is sum of the conductance in the corresponding node. 
            for c=1:r
                if any(resistors{2}(c,:)==a)%searchs each R whether it is connected to thenode a, or not.
...
for a=1:i
    if isources{1}(a)>0
        if isources{2}(a,2)>0%checks nonzero of the indexing.
            z(isources{2}(a,2))=z(isources{2}(a,2))+isources{1}(a);
        end
        if isources{2}(a,1)>0%checks nonzero of the indexing.
             z(isources{2}(a,1))=z(isources{2}(a,2))-isources{1}(a);
        end        
    else  
        if isources{2}(a,2)>0%checks nonzero of the indexing.
            z(isources{2}(a,2))=z(isources{2}(a,2))-isources{1}(a);
