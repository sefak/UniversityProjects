...
class matrix{
	...
	public:
		...
		double inf_norm_b(){ // determining the infinity norm of the vector b, but it keeps the sign info for accurate result
			double max;
			max=abs(b[0]);
			for(i=1; i<s; i++){
				if(abs(b[i])>abs(max))
...
// writing the results on the asked form
	...
	if(file_result.is_open()){
		...
		for(i=0; i<s; i++)// If the 2nd eigenvector corresponding to 2nd eigenvalues is wanted, erase this comment marks
