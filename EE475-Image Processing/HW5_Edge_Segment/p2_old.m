...
while true
    ...
    for n=1:N
        ...
        for rN = 1:r_size
            for cN = 1:c_size
                if temp_img(rN,cN,:) == -1, continue; end % if the pixel is labeled then skip it
                temp = vecnorm(temp_img(rN,cN,:)-M(n).feature);
                if temp < Th
                     M(n).index = [M(n).index; [rN cN]];
                     M(n).feature = (M(n).feature*M(n).length + temp_img(rN,cN,:)) ./ (M(n).length+1); % updating the mean of the regions
