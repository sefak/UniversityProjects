...
tap_num = 8; % determination of the tap number
if chan==1, fl=fl1; end 
if chan==2, fl=fl2; end 
if chan==3, fl=fl3; end
chan_size = length(fl);
H = ones(tap_num, (chan_size+1)/2+tap_num-1);
for n = 1:tap_num
   H(n,:) = [zeros(1,n-1) fl(1:(end+1)/2) zeros(1,tap_num-n)];
end
e  = zeros(1,(chan_size+1)/2+tap_num-1);
e(end) = 1;
...
for nEN = 1:length(snr_db) % SNR POINTS
    ...
    w_FF = [((H*H'+eye(tap_num)/en)\(H*e'))' zeros(1,tap_num-1)];
    w_FB = conv(fl((end+1)/2+1:end),w_FF,'same');
    w_FB = [w_FB zeros(1,tap_num-length(w_FB)-1)];
    nframe = 0;
    while (nframe<max_nframe) && (ferrs(nEN)<ferlim)
        ...
        equalized_bits = [conv(rec_bits, w_FF, 'same') zeros(1,tap_num-1)];
        %%%%DETECTOR %%%%%%%%%%%%
        for k=1:N
            dm = zeros(1,2);
            for r=1:2
            ...
            equalized_bits(k+1:k+tap_num-1) = equalized_bits(k+1:k+tap_num-1)-conv(w_FB,detected_bits,'same'); 
...
%save(sprintf('CH%i_DFE_%iT.mat',chan,tap_num),'sim_res');
